...
fn match_hostname(host: &str, pattern: &str) -> bool {
    for entry in pattern.split(',') {
        if entry.starts_with("|1|") {
            ...
            #[cfg(feature = "rs-crypto")]
            if let Ok(hmac) = Hmac::<sha1::Sha1>::new_from_slice(&salt) {
                if hmac.chain_update(host).verify_slice(&hash).is_ok() {
                    return true;
                }
            }
            #[cfg(feature = "openssl")]
            if let Ok(mut hasher) = openssl::hash::Hasher::new(openssl::hash::MessageDigest::sha1()) {
                hasher.update(&salt);
                hasher.update(host.as_bytes());
                if let Ok(d) = hasher.finish() {
                    if openssl::memcmp::eq(&d, &hash) {
                        return true;
                    }
                }
              }
